...
`,
		...
		expect(javascript).toContain('class="inspector__summary"');
		expect(javascript).toContain('class="criterion__text"');
		const css = await stylesheet.text();
		expect(css).toContain("--bg-base: #0a0d12");
		...
		expect(css).toContain("grid-template-columns: 42px 18px minmax(0, 1fr)");
